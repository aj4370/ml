...
class OhlcvStreamManager:
    ...
    - get_df(symbol, tf) → DataFrame or None
    ...
    def __init__(self, data_manager, is_testnet: bool = False):
        ...
        self.ws_url = Config.WS_BYBIT_TEST if is_testnet else Config.WS_BYBIT_REAL
        self._cache: dict = {}
    ...
    async def subscribe(self, symbols: list, tfs: list = None):
    ...
    async def _load_history(self, symbol: str, tf: str):
        try:
            ...
            key = (symbol, tf)
            async with self._cache_lock:
                self._cache[key] = deque(rows, maxlen=self._max_bars)
...
class TechnicalAnalyzer:
    ...
    def _check_signals_sync(ohlcvs_dict, side: str = "long"):
        ...
        AND 조건:
          ...
          4) 1m 이전봉[-2] 저가 ≤ 1m ST + ATR*0.3 (터치 허용)
          5) 1m 이전봉[-2] 양봉
          6) 1m 현재봉[-1] 양봉 (진행 중)
        초기 SL = max(min(현재1m저가, 이전1m저가), 1m_ST_현재)
        return: (sig, full_exit_sig, entry_sl, strategy_name, sl_source, common_msg, details)
        """
        try:
            ...
            if side == "long":
                ...
                cond_touch     = (l1_prev <= st1_prev + touch_buf)
                cond_bull_prev = (c1_prev > o1_prev)
                cond_bull_now  = (c1_now  > o1_now)

                details = {
                    "side": side,
                    "cond_bb5_expand": cond_bb5_expand,
                    "cond_rsi5": cond_rsi5, "rsi5": rsi5_now,
                    "cond_st1_dir": cond_st1_dir,
                    "dir1_now": dir1_now, "dir1_prev": dir1_prev,
                    "cond_touch": cond_touch,
                    "l1_prev": l1_prev, "st1_prev": st1_prev, "touch_buf": touch_buf,
                    "cond_bull_prev": cond_bull_prev,
                    "cond_bull_now": cond_bull_now,
                    "st1_now": st1_now, "atr1_now": atr1_now,
                }
...
class AsyncTradingBot:
                ...
                if use_1m_sl and is_long:
                    ...
                    if df1_sl is not None and len(df1_sl) >= 20:
                        try:
                            ...
                            pass
                        if "st" in df1_sl.columns and "st_dir" in df1_sl.columns and "atr" in df1_sl.columns:
                            st1  = float(df1_sl["st"].iloc[-1]    or 0.0)
                            dir1 = int(df1_sl["st_dir"].iloc[-1]  or 0)
                            atr1 = float(df1_sl["atr"].iloc[-1]   or 0.0)
                            atr1_k = 0.20
                            if st1 > 0 and dir1 == 1:
                    ...
                    if dfs is not None and len(dfs) >= 30:
                        ...
                        if "atr" in df.columns and "st" in df.columns and "st_dir" in df.columns:
                            ...
                            high_now = float(df["high"].iloc[-1] if "high" in df.columns else 0.0)
                            low_now = float(df["low"].iloc[-1] if "low" in df.columns else 0.0)
    ...
    async def execute_aggressive_order(self, symbol, side, qty, note, total_bal, sl_p=None, depth=None):
        ...
        try:
            ...
            # --- 근본 수정: market info에서 step_size 직접 추출 후 Decimal floor ---
            # CCXT amount_to_precision은 markets 정보 오류 시 잘못된 값 반환 가능
            # (예: NEO step=1 인데 CCXT가 0.1로 알고 있으면 qty=9.1 → Bybit 거부)
            def _floor_to_step(raw: float, sym: str) -> float:
                try:
                    mkt = self.exchange.market(sym)
                    # CCXT TICK_SIZE 모드: precision.amount = step 값 자체
                    step_val = (
                        mkt.get("precision", {}).get("amount")
                        or mkt.get("limits", {}).get("amount", {}).get("step")
                    )
                    if step_val is None:
                        return float(self.exchange.amount_to_precision(sym, raw))
                    step_f = float(step_val)
                    if step_f <= 0:
                        return float(self.exchange.amount_to_precision(sym, raw))
                    # Decimal 기반 내림(floor) — ROUND_DOWN 항상 적용
                    d_raw  = Decimal(str(raw))
                    d_step = Decimal(str(step_f))
                    n = (d_raw / d_step).quantize(Decimal("1"), rounding=ROUND_DOWN)
                    return float(n * d_step)
                except Exception:
                    return float(self.exchange.amount_to_precision(sym, raw))

            qty_prec = _floor_to_step(qty_raw, symbol)
            min_q = float(self.get_symbol_min_amount(symbol) or 0.0)

            if qty_prec <= 0:
                self.queue_notify(f"[ENTRY_BLOCK] {symbol} qty=0 (raw={qty_raw:.6f}, min={min_q})")
                return

            if min_q > 0 and qty_prec < min_q:
                self.queue_notify(f"[WARN] {symbol} qty<{min_q} → minQty로 보정 (qty:{qty_prec}→{min_q})")
                qty_prec = _floor_to_step(min_q, symbol)
                if qty_prec <= 0:
            ...
            try:
                if sl_sanitized and float(sl_sanitized) > 0 and float(target_p) > 0:
                    ...
                    # 리스크 초과 방지: 더 작아지면 축소 (_floor_to_step 동일 적용)
                    if qty2 and float(qty2) > 0:
                        qty2_prec = _floor_to_step(float(qty2), symbol)
    ...
    async def process_symbol(self, symbol, total_bal, side: str = "long", common_msg: str = ""):
        ...
        async with self._get_entry_lock(symbol):
            try:
                ...
                # 데이터 fetch — stream_manager 캐시 우선, REST fallback
                dfs = {}
                sm = getattr(self, "stream_manager", None)
                if sm is not None:
                ...
                elif df5 is not None and len(df5) >= 2:
    ...
    async def run_loop(self):
        ...
        while self.is_running:
            try:
                ...
                # -------------------------
                # 2) 공통조건 — CommonFilterCache 캐시 우선
                #    캐시 히트: 즉시 사용 / 캐시 미스: 백그라운드 갱신 후 이번 루프 스킵
                # -------------------------
                common_map = {}
                cache_miss_syms = []
                ...
                # 엔트리 시그널 체크 (배치, 딜레이 없음)
                tasks = [
                    self.process_symbol(sym, total_bal, side=side, common_msg=msg)
                    for sym, msg in eligible
                ]
